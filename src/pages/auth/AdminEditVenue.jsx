...
function AdminEditVenue() {
    ...
    const getVenue = async () => {
      try {
        ...
        setForm({
          ...
          imagePublicId: venue.imagePublicId || "",
        });
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await service.put(`/venues/${id}`, payload);
      ...
      toast.error(
        error.response?.data?.errorMessage || "Failed to update venue",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 p-10">
      <div className="max-w-4xl mx-auto">
        {/* HEADER */}
        <h1 className="text-4xl font-bold mb-2">Edit Venue</h1>

        <p className="text-zinc-400 mb-8">Update venue details and settings</p>

        <form
          onSubmit={handleSubmit} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8 space-y-8">
