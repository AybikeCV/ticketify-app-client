...
function Profile() {
  const { loggedUserId, loggedUserRole, logoutUser, loggedUserName } =
  ...
  const getBookings = async () => {
    try {
      const response = await service.get("/bookings/mybookings");
  ...
  const handleDeleteProfile = async () => {
    try {
      await service.delete("/users/profile");
      ...
      toast.error(
        error.response?.data?.errorMessage || "Failed to delete profile",
      );
    }
  };
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8">
          ...
          <div className="mt-8 space-y-4">
            ...
            <div>
              ...
              <p className="text-[#1B5E4A] capitalize">{loggedUserRole}</p>
            </div>
