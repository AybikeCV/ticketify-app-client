...
function ConcertCard({ concert }) {
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden hover:border-[#1B5E4A]/40 transition">
      ...
      <div className="p-5">
        <h2 className="text-xl font-semibold text-zinc-100">{concert.title}</h2>

        <p className="text-xl font-semibold text-[#1B5E4A] mt-1">
          {concert.artist}
        </p>

        <p className="text-zinc-400 text-m mt-3">
          {new Date(concert.date).toLocaleDateString()}
        </p>

        <p className="text-zinc-500 text-m bold">
          <Link to={`/venues/${concert.venue._id}`}>{concert.venue.name}</Link>
        </p>

        <div className="flex items-center justify-between mt-5">
          <span className="text-zinc-100 font-medium">€{concert.price}</span>
